...
import { useAffiliateForgotPasswordMutation } from '@/lib/api/authApi';
...
export default function AffiliateForgotPasswordPage() {
  ...
  const [forgotPassword, { isLoading }] = useAffiliateForgotPasswordMutation();
    ...
    try {
      ...
      if (result.success) {
        ...
        if (typeof window !== 'undefined') {
          localStorage.setItem('affiliate_reset_password_email', formData.email);
        }
        ...
        setTimeout(() => {
          router.push(`/${locale}/affiliate/reset-password?email=${formData.email}`);
        }, 2000);
      }
    } catch (error) {
