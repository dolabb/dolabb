...
import { useAffiliateResetPasswordMutation } from '@/lib/api/authApi';
...
export default function AffiliateResetPasswordPage() {
  ...
  const [resetPassword, { isLoading }] = useAffiliateResetPasswordMutation();

  useEffect(() => {
    // Get email from localStorage if not in params
    if (!email && typeof window !== 'undefined') {
      const storedEmail = localStorage.getItem('affiliate_reset_password_email');
      if (storedEmail) {
        setEmail(storedEmail);
      } else {
        // No email found, redirect to forgot password
        router.push(`/${locale}/affiliate/forgot-password`);
      }
    }
  }, [email, locale, router]);
  ...
  const handleOtpChange = (index: number, value: string) => {
    ...
    if (value && index < 3) {
      const nextInput = document.getElementById(`affiliate-reset-otp-${index + 1}`);
      nextInput?.focus();
    }
  };
  ...
  const handleKeyDown = (index: number, e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === 'Backspace' && !otp[index] && index > 0) {
      const prevInput = document.getElementById(`affiliate-reset-otp-${index - 1}`);
      prevInput?.focus();
    }
  };
  ...
  const handlePaste = (e: React.ClipboardEvent) => {
    ...
    if (/^\d{4}$/.test(pastedData)) {
      ...
      setOtp(newOtp);
      // Focus last input
      document.getElementById('affiliate-reset-otp-3')?.focus();
    }
  };
    ...
    try {
      ...
      if (result.success) {
        ...
        if (typeof window !== 'undefined') {
          localStorage.removeItem('affiliate_reset_password_email');
        }
