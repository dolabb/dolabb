...
export const categoryBrands: Record<string, string[]> = {
  'women': ['Gildan', 'Nike', 'American Vintage', 'Harley Davidson', 'Zara', 'H&M', 'Forever 21'],
  'men': ['Gildan', 'Nike', 'American Vintage', 'Harley Davidson', 'Adidas', 'Levi\'s', 'Uniqlo'],
...
const generateWardrobeEssentialsProducts = (): CategoryProduct[] => {
  ...
  subcats.forEach((subcat, subcatIndex) => {
    brands.forEach((brand, brandIndex) => {
      for (let i = 0; i < 3; i++) {
        const productIndex = subcatIndex * brands.length * 3 + brandIndex * 3 + i;
        products.push({
          id: `we-${productIndex}`,
          category: 'women',
          subCategory: subcat.toLowerCase().replace(/\s+/g, '-'),
          brand: brand,
          image: `https://images.unsplash.com/photo-${imageIds[productIndex % imageIds.length]}?w=500&h=500&fit=crop&auto=format`,
          title: `${brand} ${subcat}`,
          price: 15.99 + (productIndex % 10) * 5,
          originalPrice: productIndex % 3 === 0 ? 25.99 + (productIndex % 10) * 5 : undefined,
          ...
          condition: ['new', 'like-new', 'good', 'fair'][productIndex % 4] as any,
        });
      }
    });
  });
  ...
  brands.forEach((brand, brandIndex) => {
    for (let i = 0; i < 8; i++) {
      ...
      products.push({
        ...
        subCategory: 'tshirts',
        brand: brand,
        image: `https://images.unsplash.com/photo-${imageIds[productIndex % imageIds.length]}?w=500&h=500&fit=crop`,
        title: `${brand} Men's T-shirt`,
        price: 19.99 + (productIndex % 10) * 3,
        originalPrice: productIndex % 4 === 0 ? 29.99 + (productIndex % 10) * 3 : undefined,
...
export const getCategoryData = (category: string, subCategory: string): CategoryData | null => {
  const key = `${category}-${subCategory}`;
  return categoryProductsData[key] || null;
};

// Helper function to get all products for a category
export const getCategoryProducts = (category: string, subCategory: string): CategoryProduct[] => {
